...
$sql='select cat_id from category where parent_id=1';
$b=$pdo->prepare($sql);
$b->execute(); 
//$rel是二维数组
//2个产品
//一维数组：产品信息
//pro_id/pro_name...
$rels=$b->fetchAll(PDO::FETCH_ASSOC);
$data=array();
foreach($rels as $rel){
	$sql='select cat_id from category where parent_id=?';
	$b=$pdo->prepare($sql);
	$b->execute(array($rel['cat_id'])); 
	$rel2s=$b->fetchAll(PDO::FETCH_ASSOC);
	foreach($rel2s as $rel2){
		$sql2='select * from product where cat_id=?';
		$p=$pdo->prepare($sql2);
		$p->execute(array($rel2['cat_id']));
		$rel3=$p->fetchAll(PDO::FETCH_ASSOC);
		$data[]=$rel3;
	}
}
?>
